...
__device__ double delta_r(double r) {
    return r * r + a * a - 2.0 * M * r;
}

__device__ double ddelta_r(double r) {
    return 2.0 * r - 2.0 * M;
}
...
__device__ double E_func(double r, double theta, double dr, double dtheta, double dphi) {
    double sin_theta = sin(theta);
    sin_theta = fmax(sin_theta, epsilon);
    double delta = delta_r(r);
    double term = ((a * a * sin_theta * sin_theta - delta) * (-dr * dr / delta - dtheta * dtheta)
...
__device__ double L_func(double r, double theta, double dphi, double E) {
    ...
    double num = a * E * delta + (sigma_val * delta * dphi - a * E * (r * r + a * a));
    double denom = delta - a * a * sin_theta * sin_theta;
    return sin_theta * sin_theta * num / denom;
}

__device__ double k_func(double r, double theta, double dr, double E, double L) {
    double sigma_val = sigma(r, theta);
    double delta = delta_r(r);
    double W = W_r(r, E, L);
...
__device__ double dp_r(double r, double theta, double p_r, double E, double L, double k_val) {
    double sig = sigma(r, theta);
    double delta = delta_r(r);
...
__global__ void simulateRayKernel(int num_rays, const double *init_conditions, double *trajectories, int *steps_out, int num_steps) {
...
// (num_rays * num_steps * 5 double values) and the number of steps for each ray.
extern "C" {

#ifdef _WIN32
__declspec(dllexport)
#endif
void simulateRays(size_t num_rays, size_t num_steps, const double *init_conditions_host, double *trajectories_host, int *steps_out_host) {
    ...
    cudaMemcpy(d_init_conditions, init_conditions_host, init_size, cudaMemcpyHostToDevice);
    ...
    simulateRayKernel<<<blocks, threadsPerBlock>>>((int)num_rays, d_init_conditions, d_trajectories, d_steps_out, (int)num_steps);
    cudaDeviceSynchronize();


    // Copy the results back to host.
    cudaMemcpy(trajectories_host, d_trajectories, traj_size, cudaMemcpyDeviceToHost);
